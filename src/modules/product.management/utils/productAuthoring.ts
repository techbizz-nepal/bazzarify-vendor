...
export const prepareProductSubmission = <TSchema extends z.ZodTypeAny>({
  ...
  if (!hasCustomerSelectableOptions) {
    const optionlessVariant = variantData[OPTIONLESS_VARIANT_KEY] || {};

    if (!isValidOptionlessVariant(optionlessVariant)) {
      return {
        ok: false,
        kind: "variants",
        message: "Please fill SKU, stock, and price for the internal SKU.",
        feedback: {
          summary: "Please complete the internal SKU fields before submitting.",
          fieldErrors: {},
        },
        updatedVariantData: variantData,
      };
    }
    ...
    const variants = createOptionlessVariantPayload({
      ...
      productSku,
      variant: optionlessVariant,
    });
    const validationResult = schema.safeParse({
      ...product,
      base_price: optionlessVariant.price || product.base_price,
    ...
    appendFormDataVariants(formData, variants, [], categoryAttributes);
